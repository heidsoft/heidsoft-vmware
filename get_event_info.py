...
def main():
   ...
   ids = ['VmRelocatedEvent', 'DrsVmMigratedEvent', 'VmMigratedEvent','HostEvent']
   eMgrRef = content.eventManager
   #filter = vim.event.EventFilterSpec.ByEntity(entity=hosts, recursion="self")                
   #filterSpec = vim.event.EventFilterSpec(entity=filter, eventTypeId=ids) 

   filterSpec = vim.event.EventFilterSpec()        
   event_list = eMgrRef.QueryEvents(filterSpec)
   event_History_Collector = eMgrRef.CreateCollectorForEvents(filterSpec)
   #eMgrRef.LogUserEvent("event-test")                 
   #print event_History_Collector.latestPage
